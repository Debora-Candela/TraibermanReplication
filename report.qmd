...
Since the Danish administrative data used in the original paper is strictly confidential, this replication focuses on validating the first-stage Expectation-Maximization (EM) procedure using a synthetic Data Generating Process (DGP) designed to align wtih the paper's theoretical framework.
...
Moreover, to rigorously test the EM algorithm, the synthetic DGP (`src/DGP.jl`) was constructed to satisfy the structural assumptions of the paper's labor supply model:
...
In this phase, the current posterior type probabilities (`PI_TYPE`) are treated as given weights to estimate the structural parameters via Weighted Least Squares (WLS):
...
The posterior probability is then updated as:
$$
...
* **Algebraic Stabilization in Bayes' Rule:** A naive computation of the Bayesian update can easily lead to floating-point overflow (`Inf` errors) due to the exponentiation of large likelihood values. To preserve numerical integrity, the formula is algebraically transformed by dividing both the numerator and denominator by $\exp(L_{i1})$:
    $$q_{i1} = \frac{\pi_1}{\pi_1 + \pi_2 \exp(L_{i2} - L_{i1})}$$
    This adjustment (`exp.(L[:, 2] .- L[:, 1])` in the script) keeps the computations strictly bounded within standard 64-bit architecture limits while remaining mathematically identical to the theoretical update.
---
...
results = TraibermanReplication.run()
```

produces a structured output `(:q1, :beta, :sigma, :data)` containing the core structural estimates of the model:
...
results = redirect_stdio(stdout=devnull, stderr=devnull) do
    TraibermanReplication.run()
end

table4 = TraibermanReplication.table4_like(results)

println("| Type | Classification | Mean theta | Mean income (relative) | corr(CA, wage) |")
println("|:---:|:---:|:---:|:---:|:---:|")
for r in eachrow(table4)
    theta = round(r.Mean_theta, digits=3)
    inc = round(r.Mean_income_relative, digits=3)
    corr = round(r.corr_CA_wage, digits=3)
    
    println("| $(r.Type) | $(r.Classification) | $theta | $inc | $corr |")
end
```
